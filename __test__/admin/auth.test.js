...
beforeAll(async function (){
  try {
    ...
    const db = client.db('sad_test');

    const user = db.collection('user');
    inserted_user = await user.insertOne({
      username: 'Sherwood_Wintheiser25',
      password: '04DJG11nH3I2x4J',
      email: 'Micaela_Johnson@yahoo.com',
      name: 'Lonnie Bradtke',
      role: 208,
      resetPasswordLink: {},
      loginRetryLimit: 942,
      loginReactiveTime: '2022-02-27T10:40:48.128Z',
      id: '616d08bac7b2eb2504795a0f'
    });
  }
  catch (err) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Cloyd_Harber98',
        'password':'njnHeiiWvtlJxg6',
        'email':'Bobby73@hotmail.com',
        'name':'Omar Leffler',
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/admin/auth/forgot-password')
      .send({ 'email':'Bobby73@hotmail.com', });
