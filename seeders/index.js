...
async function seedUserRole (){
  try {
    let user = await User.findOne({
      'email':'Lupe22@yahoo.com',
      'isActive':true,
      'isDeleted':false
    });
    let userRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (user && user.isPasswordMatch('L_NrDqQ3LfYptRC') && userRole){
    ...
    let admin = await User.findOne({
      'email':'Finn_Bayer@yahoo.com',
      'isActive':true,
      'isDeleted':false
    });
    let adminRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (admin && admin.isPasswordMatch('S2fpd4xf2UMGiLd') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id,
        isActive: true, 
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('L_NrDqQ3LfYptRC') ) {
      let user = new User({
    ...
    if (!admin || !admin.isPasswordMatch('S2fpd4xf2UMGiLd') ) {
      let admin = new User({
